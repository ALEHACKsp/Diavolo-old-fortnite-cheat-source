...
namespace rend {
	...
	VOID EndScene(ImGuiWindow& window) 
	{
		...
		if (showMenu)
		{
			...
			if (ImGui::Begin(xorstr("Diavolo cheat  V1 BY FN-Diavolo#3472"), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_AlwaysAutoResize))
			...
			ImGui::Tab(0, xorstr("visuals"), &tab, 260);
			ImGui::Tab(1, xorstr("aimbot"), &tab, 260);
			ImGui::Tab(4, xorstr("Menu normal"), &showmenu2, 260);
		...
		if (showmenu2)
		{
			...
			//	ImGui::Checkbox(xorstr("spinbot##checkbox"), &Settings.Spinbot);
				...
				ImGui::Checkbox(xorstr("Speed hack )##checkbox"), &Settings.CustomSpeedHack);
...
*/
				for (auto li = 0UL; li < ReadDWORD(world, offsets::World::Levels + sizeof(PVOID)); ++li) {
					...
					for (auto ai = 0UL; ai < ReadDWORD(level, offsets::Level::AActors + sizeof(PVOID)); ++ai) {
						...
						else if (wcsstr(name.c_str(), xorstr(L"FortPickupAthena")))
						{
						...
						else if (Settings.fishingholeesp && wcsstr(name.c_str(), xorstr(L"BGA_Athena_FlopperSpawn_C"))) {
							...
							if (LLamaRoot) {
								...
								if (Util::WorldToScreen(width, height, &LLamaPos.X)) {
									...
									if (dist < 5000)
									{
										DrawOutlinedText(m_pFont, TextFormat(xorstr("Floper Spawn [%.0f m]"), dist), ImVec2(LLamaPos.X, LLamaPos.Y), 14.5f, ImGui::GetColorU32({ 210, 0, 240, 255 }), true);
									}
								}
							}
						}
